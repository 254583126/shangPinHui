    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            <!-- 分类 -->
            <li class="with-x" v-if="searchParams.categoryName">{{ searchParams.categoryName }}<i @click="removeCategroyName">x</i></li>
            <!-- 关键字 -->
            ...
            <!-- 属性值 -->
            <li class="with-x" v-for="(attrVal,index) in searchParams.props" :key="index">{{ attrVal.split(':')[1]}}<i @click="removeAttrVal(index)">x</i></li>
          </ul>
        </div>
...
<script>
  ...
  export default {
    ...
    watch:{
      //监视$route数据实现用户点击搜索或其他选项后重新发送请求
      $route(newVal,oldVal){
        //每次请求前重新更新请求数据
        Object.assign(this.searchParams,this.$route.query,this.$route.params)
        this.getData()
