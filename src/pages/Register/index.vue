<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="controls">
        <input type="checkbox" v-model="isagre" name="isagre" v-validate="{ required: true, 'agree': ture }"
          :class="{ invalid: errors.has('isagre') }" />
        <span>同意协议并注册《尚品汇用户协议》</span>
        <span class="error-msg">{{ errors.first("isagre") }}</span>
      </div>
      <div class="btn">
...
export default {
  ...
  methods: {
    ...
    async toRegister() {
      ...
      if (success) {
        try {
          const { phone, code, password, password1, isagre } = this;    //这里必须加分号
