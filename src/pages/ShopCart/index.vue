  ...
  <div class="cart">
      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(cartInfo,index) in cartInfoList" :key="cartInfo.id">
          <li class="cart-list-con1">
            <input type="checkbox" name="chk_list" :checked="cartInfo.isChecked==1" @change="updateCheckByid(cartInfo,$event)">
          </li>
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input class="chooseAll" type="checkbox" :checked="isAllCheck && cartInfoList.length>0" @change="updataCheckAll">
        <span>全选</span>
      </div>
      <div class="option">
...
import throttle from 'lodash/throttle';
  export default {
    ...
    methods:{
      ...
      //改变数量时的回调 --- type:区分元素，disnum：变化量，cartInfo：上面存储着id   throttle:节流
  ...
  this.getData()
    ...
    alert(error.message)
     ...
     async deleteCartById(cartInfo){
      ...
      //更改选中状态
     async updateCheckByid(cartInfo,event){
      try {
        let isChecked  = event.target.checked?1:0
        await this.$store.dispatch('updateIschecked',{skuId:cartInfo.skuId,isChecked})
        this.getData()
      } catch (error) {
        alert(error.message)
      }
      ...
      //全选按钮
      async updataCheckAll(event){
        try {
          let isChecked = event.target.checked?'1':'0'
          await this.$store.dispatch('updataCheckAll',isChecked)
          this.getData()
        } catch (error) {
          alert(error.message)
        }
